...
・映画情報の検索、登録、修正、削除ができるため、下記用途に使えます。<br>①気になる映画をまとめておくために利用<br>
②これから視聴予定の映画を<br>③修正機能を活用して、映画情報が一部変更になった時に利用
...
具体的に同じ映画名の登録をリクエストしてきた場合はステータスコード409とエラーメッセージを返すように実装。<br>
文字の間隔は関係なく、純粋に映画名が一致していたら例外が適用される。<br>
...
うまく実装できた場合はステータスコード200とメッセージを返すようにする。<br>
